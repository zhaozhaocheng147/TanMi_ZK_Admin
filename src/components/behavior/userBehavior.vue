<template>
  <ion-header :translucent="true">
    <ion-toolbar style="--background:#f4f4f5;">
      ...
      <ion-title>用户低碳行为提交信息</ion-title>
    </ion-toolbar>
  </ion-header>
  ...
  <ion-content>
    <div style="position: relative">
      <ion-searchbar placeholder="行为提交日志Id检索" v-model="searchInfo"
                     @ionChange="getIdTasks()"  debounce="1200">
      </ion-searchbar>
    ...
    <div style="width: 120px; margin: 0 auto;" v-if="noInfo">没有相关行为</div>
          ...
          <ion-card>
            <ion-card-content>
              ...
              <ion-grid>
                ...
                <ion-row>
                  <ion-col size="4">低碳行为名称</ion-col>
                  <ion-col size="8" class="rolStyle">{{ item.tName }}</ion-col>
                </ion-row>
                <ion-row>
                  <ion-col size="4" >提交描述</ion-col>
                  <ion-col size="8" class="rolStyle">{{ item.tDesc }}</ion-col>
                </ion-row>
                <ion-row>
                ...
                <ion-row>
                  ...
                  <ion-col size="8" class="rolStyle" v-if="item.tState == -1" style="color: green">用户已撤回</ion-col>
                </ion-row>
...
export default defineComponent({
  ...
  methods: {
    ...
    getFailTasks(){
      ...
      let _this = this;
      $.ajax({
        url: 'https://tanmi-api.rexue.plus/behaviors/log/all',
        type: 'get',
        data: {
        },
        beforeSend: function (request) {
    ...
    getIdTasks(){
      ...
      let _this = this;
      $.ajax({
        url: 'https://tanmi-api.rexue.plus/behaviors/log/'+_this.searchInfo,
        type: 'get',
        data: {
        },
        beforeSend: function (request) {
